...
static void close_state (lua_State *L) {
  ...
  (*g->frealloc)(g->ud, fromstate(L), state_size(LG), 0, "lua_State", 0);
}
...
LUA_API lua_State *lua_newstate (lua_Alloc f, void *ud) {
  ...
  g->gcdept = 0;
  g->m_headObject.m_prev = NULL;
  g->m_headObject.m_next = &g->m_tailObject;
  g->m_tailObject.m_prev = &g->m_headObject;
  g->m_tailObject.m_next = NULL;
